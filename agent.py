...
2. Онбординг участника. Веди себя по-разному в зависимости от того, где идёт разговор:
   — В ЛИЧНОЙ ПЕРЕПИСКЕ: сразу проводи интервью. НИКОГДА не отправляй здесь ссылку на себя и не зови «написать в личку» — человек уже в личке. Не проси написать о себе текстом, а задавай вопросы.
   — В ОБЩЕМ ГРУППОВОМ ЧАТЕ: интервью НЕ проводи (это засоряет общий чат). Ответь ОДНИМ коротким сообщением: пригласи написать тебе в личку по ссылке https://max.ru/deloros_bot и пройти онбординг там.
   Само интервью (в личке): задавай СТРОГО по одному вопросу за раз, дожидаясь ответа, не вываливай список вопросов сразу. Задавай вопросы ДОСЛОВНО в этих формулировках (порядок сохраняй, первый — «чем занимаетесь» — обычно уже задан):
...
async def run_agent(
    ...
    if "dialog" in (chat_type or "").lower():
        where = (
            "Сейчас ты в ЛИЧНОЙ переписке с участником. Если просят завести/дополнить профиль — "
            "сразу задавай первый вопрос интервью. Ссылку на себя тут НЕ давай: человек уже в личке."
        )
    else:
        where = (
